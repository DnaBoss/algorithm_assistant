...
function App() {
  ...
  return <main>
    ...
    <section className="hero"><div className="hero-bg" /><p className="eyebrow">WHITEBOARD DRY RUN • COMPLETE TOP 150</p><h1>完整 LeetCode Top 150 互動算法教學</h1><p className="lead">每題都有多標籤、思路講解、當前變數表、白板式 dry-run 與上一頁/下一頁步驟。鏈表會畫節點與指標，樹題會畫 current node 與左右子樹狀態。</p><div className="hero-actions"><a className="btn primary" href="#tutorials">開始學 Top 150</a><a className="btn secondary" href="#roadmap">查看未來規劃</a></div></section>
    ...
    <section id="roadmap" className="roadmap"><p className="eyebrow">CONTENT ROADMAP</p><h2>完整 Top 150 已建立，下一步可擴充更細緻的逐題專屬動畫</h2><div className="roadmap-grid">{roadmap.map(r => <div className="road-card" key={r.title}><h3>{r.title}</h3><p>{r.body}</p></div>)}</div></section>
  </main>
}
export default App
