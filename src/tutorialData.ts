...
const codeByPattern: Record<VisualKind, string[]> = {
  ...
  'linked-list': ['const dummy = new ListNode(0)', 'let prev = dummy, cur = head', '調整 next 指標，避免遺失後續節點', '移動 prev / cur 到下一個白板狀態', 'return dummy.next ?? head'],
  tree: ['function dfs(node) {', '  if (!node) return base', '  const left = dfs(node.left)', '  const right = dfs(node.right)', '  return combine(node, left, right)', '}'],
...
function specificSteps(raw: RawTutorial): Step[] | null {
  if (raw.id === 'two-sum') return [
    { title: 'LeetCode 測資與初始化', explain: '使用 LeetCode 範例 nums=[2,7,11,15], target=9。map 先是空的，還沒進入迴圈。', codeLine: 'const map = new Map<number, number>()', variables: { nums: '[2,7,11,15]', target: 9, i: '-', nums_i: '-', need: '-', map: '{}', answer: '-' }, visual: { kind: 'array', items: [2, 7, 11, 15], pointers: [], notes: ['測資固定：nums=[2,7,11,15], target=9', 'map 記錄「值 → 索引」'] } },
    { title: 'i=0，讀 nums[0]', explain: '迴圈開始，i 指向 index 0，目前值 nums[i]=2。', codeLine: 'for (let i = 0; i < nums.length; i++) {', variables: { nums: '[2,7,11,15]', target: 9, i: 0, nums_i: 2, need: '-', map: '{}', answer: '-' }, visual: { kind: 'array', items: [2, 7, 11, 15], pointers: [{ label: 'i', index: 0, color: '#18E299' }], notes: ['目前處理 nums[0] = 2'] } },
    { title: '計算 need=7', explain: '照程式碼計算 need = target - nums[i] = 9 - 2 = 7。', codeLine: '  const need = target - nums[i]', variables: { nums: '[2,7,11,15]', target: 9, i: 0, nums_i: 2, need: 7, map: '{}', answer: '-' }, visual: { kind: 'array', items: [2, 7, 11, 15], pointers: [{ label: 'i=0', index: 0, color: '#18E299' }, { label: 'need=7', index: 1, color: '#fbbf24' }], notes: ['need=7 但 map 目前是空的'] } },
    { title: '查 map，沒有 7', explain: '執行 if (map.has(need))，map 裡沒有 7，所以不能回傳答案。', codeLine: '  if (map.has(need)) return [map.get(need)!, i]', variables: { nums: '[2,7,11,15]', target: 9, i: 0, nums_i: 2, need: 7, map: '{}', hasNeed: false, answer: '-' }, visual: { kind: 'array', items: [2, 7, 11, 15], pointers: [{ label: 'i=0', index: 0, color: '#18E299' }, { label: 'not found', index: 1, color: '#ef4444' }], notes: ['map.has(7) = false'] } },
    { title: '把 2 放進 map', explain: '沒有找到補數，所以執行 map.set(nums[i], i)，記錄 2 的索引是 0。', codeLine: '  map.set(nums[i], i)', variables: { nums: '[2,7,11,15]', target: 9, i: 0, nums_i: 2, need: 7, map: '{2:0}', answer: '-' }, visual: { kind: 'array', items: [2, 7, 11, 15], pointers: [{ label: 'stored', index: 0, color: '#18E299' }], notes: ['map = {2:0}', '下一輪若需要 2，就能找到 index 0'] } },
    { title: 'i=1，讀 nums[1]', explain: 'i 前進到 index 1，目前值 nums[i]=7。', codeLine: 'for (let i = 0; i < nums.length; i++) {', variables: { nums: '[2,7,11,15]', target: 9, i: 1, nums_i: 7, need: '-', map: '{2:0}', answer: '-' }, visual: { kind: 'array', items: [2, 7, 11, 15], pointers: [{ label: 'seen', index: 0, color: '#a78bfa' }, { label: 'i', index: 1, color: '#18E299' }], notes: ['map 已經記住 2 在 index 0'] } },
    { title: '計算 need=2 並命中', explain: 'need = 9 - 7 = 2。map 裡有 2，代表 nums[0] + nums[1] = 9。', codeLine: '  const need = target - nums[i]', variables: { nums: '[2,7,11,15]', target: 9, i: 1, nums_i: 7, need: 2, map: '{2:0}', hasNeed: true, answer: '-' }, visual: { kind: 'array', items: [2, 7, 11, 15], pointers: [{ label: 'need=2', index: 0, color: '#fbbf24' }, { label: 'i=1', index: 1, color: '#18E299' }], notes: ['map.has(2) = true', 'map.get(2) = 0'] } },
    { title: '回傳答案 [0,1]', explain: '執行 return [map.get(need)!, i]，也就是 [0,1]。', codeLine: '  if (map.has(need)) return [map.get(need)!, i]', variables: { nums: '[2,7,11,15]', target: 9, i: 1, nums_i: 7, need: 2, map: '{2:0}', answer: '[0,1]' }, visual: { kind: 'array', items: [2, 7, 11, 15], pointers: [{ label: 'answer[0]', index: 0, color: '#18E299' }, { label: 'answer[1]', index: 1, color: '#18E299' }], notes: ['nums[0] + nums[1] = 2 + 7 = 9', '答案是索引，不是數值：[0,1]'] } }
  ]
...
function codeFor(raw: RawTutorial): string[] {
  if (raw.id === 'two-sum') return ['const map = new Map<number, number>()', 'for (let i = 0; i < nums.length; i++) {', '  const need = target - nums[i]', '  if (map.has(need)) return [map.get(need)!, i]', '  map.set(nums[i], i)', '}']
